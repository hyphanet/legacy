 ...
 * Version 1.4
 * 
 * A Java class to determine image width, height and color depth for a number
 ...
 * per pixel are only read from the global header. For some GIFs, local
 * palettes change this to a typically larger value. To be certain to get the
 * correct color depth, call setDetermineImageNumber(true) before calling
 * check(). The complete scan over the GIF file will take additional time.
 * </li>
 ...
 * new method getMimeType() to return the MIME type associated with a
 * particular file format.</li>
 * <li><strong>2002-03-15 </strong> Added support to recognize number of
 * images in file. Only works with GIF. Use {@link #setDetermineImageNumber}
 * with <code>true</code> as argument to identify animated GIFs (
 ...
 * the physical resolution could not be retrieved, these methods return <code>-1</code>.
 * </li>
...
public class ImageInfo {
     ...
     * extract comments from GIFs and count the number of images (GIFs with
     * more than one image are animations). If you know of a place where GIFs
     * store the physical resolution of an image, please <a
     ...
     * resolution can be stored. <em>The specification that I have says that the values must be
    ...
    /**
     * The names of all supported file formats. The FORMAT_xyz int constants
     * can be used as index values for this array.
     */
    ...
    private boolean checkJpeg() throws IOException {
        ...
        while (true) {
            ...
            if ((marker & 0xff00) != 0xff00) { return false; // not a valid
                                                             // marker
            }
            if (marker == 0xffe0) { // APPx
                if (size < 14) { return false; // APPx header must be >= 14
                                               // bytes
                }
     ...
     * the FORMAT_xyz constants from this class. Use {@link #getFormatName()}
    ...
    /**
     * Returns the number of images in the examined file. Assumes that <code>setDetermineImageNumber(true);</code>
     * was called before a successful call to {@link #check()}. This value can
     * currently be only different from <code>1</code> for GIF images.
     * 
     ...
     * Assumes that {@link #check()}was successful. Returns <code>-1</code>
     * on failure.
     * 
     ...
     * file names as parameters (information on them will be printed to
     * standard output, one line per file) or call it with no parameters. It
     * will then check data given to it via standard input.
     * 
     * @param args
    ...
    private static void printCompact(String sourceName, ImageInfo imageInfo) {
        System.out.println(imageInfo.getFormatName() + ";" + imageInfo.getMimeType() + ";" + imageInfo.getWidth() + ";" + imageInfo.getHeight() + ";"
                + imageInfo.getBitsPerPixel() + ";" + imageInfo.getNumberOfImages() + ";" + imageInfo.getPhysicalWidthDpi() + ";"
                + imageInfo.getPhysicalHeightDpi() + ";" + imageInfo.getPhysicalWidthInch() + ";" + imageInfo.getPhysicalHeightInch());
    }
    ...
    /**
     * Specify whether textual comments are supposed to be extracted from
     * input. Default is <code>false</code>. If enabled, comments will be
     ...
     * more than one image. If this method is called with <code>true</code>
     * as argument, the actual number of images can be queried via
