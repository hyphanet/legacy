...
public class Yarrow extends RandomSource {
	...
	public Yarrow(File seed, String digest, String cipher,boolean updateSeed) {
        ...
        try {
            ...
            Core.logger.log(this, "Could not init pools trying to getInstance("+digest+"): "+e, e, Logger.ERROR);
            System.exit(17);
        }
		entropy_init(seed);
	    ...
	    if(File.separatorChar == '/') {
	        ...
	        if(hwrng.exists() && hwrng.canRead()) {
		        try {
		            ...
		            Core.logger.log(this, "Can't read /dev/hwrng even though exists and is readable: "+t, t, Logger.NORMAL);
		        } finally {
		            if(fis != null) try {
		                fis.close();
		            } catch (IOException e) {
	        ...
	        try {
	            ...
	            Core.logger.log(this, "Can't read /dev/urandom: "+t, t, Logger.NORMAL);
	        } finally {
	            if(fis != null) try {
                    fis.close();
                } catch (IOException e) {
	...
	/**
     * Read some random files
     */
    private void readEntropyFromFiles() {
